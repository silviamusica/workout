# PROGRAMMA COMPLETO V4 — Revisione 2026-04-22
## Sottomesso all'agente PT per conferma

---

## STRUTTURA SETTIMANA

| Giorno | Sigla | Tipo | Esercizi | Durata |
|---|---|---|---|---|
| **Lunedì** | G1 | Pesi — Lower A | Squat, Stacco Rumeno, Glute Bridge Bilanciere, Leg Curl al Cavo, Ab Wheel | ~85 min |
| **Martedì** | G2 | Pesi — Upper A | Trazioni, Panca, Pulley, Band Pull-Apart con rotazione esterna, Curl Bicipiti | ~90 min |
| **Mercoledì** | G3 | Leggero | Alzate Laterali, Band Pull-Apart, Fire Hydrant/Clamshell, Goblet Squat, TRX Row + Protocollo anca | ~35 min |
| **Giovedì** | Cardio | Cardio programmato | Corsa 30 / HIIT upper+core / Corsa lunga / Rucking + Mobilità | 30-60 min |
| **Venerdì** | G4 | Pesi — Lower B | Stacco da Terra, Affondi, Hyperextension, Fire Hydrant | ~90 min |
| **Sabato** | G5 | Pesi — Upper B | T-bar Row, Military Press, Push-Up, Trazioni Supine, Woodchop, Alzate Laterali | ~80 min |
| **Domenica** | Riposo | Recupero | — | — |

---

# CARDIO PROGRAMMATO — Giovedì

L'app propone una rotazione A -> B -> C -> D, ma tutti i formati restano sempre selezionabili.

| Rotazione | Formato | Durata | Intensità | Note |
|---|---|---|---|---|
| A | Corsa 30 min zona 2 | 30 min | Bassa | FC 120-140, puoi parlare senza fiatone |
| B | HIIT upper + core | 30 min | Alta | 30s lavoro / 15s pausa, 4-5 giri |
| C | Corsa lunga zona 2 | 50-60 min | Bassa | Ritmo costante, cadenza 170+ |
| D | Rucking | 45-60 min | Bassa-media | Zaino 15-20 kg |

HIIT consentito: kettlebell swing leggero, push-up, battle rope, plank, shoulder tap, mountain climber, dead bug dinamico.

HIIT vietato il giovedì: sprint, box jump, burpee completo, affondi saltati, squat jump, corsa in salita.

Mobilità post-cardio invariata: dorsiflessione, 90/90, T-spine rotation, half kneeling lunge, figure-four.

---

# GIORNI PESI — Logica Split AM/PM + Superset

## 📍 GIORNO 1 — LOWER A (Lunedì)

**Durata totale: ~85 minuti**
**Struttura:** Split AM/PM con superset nel blocco PM

### RISCALDAMENTO (12-15 min)

| Fase | Esercizio | Serie/Reps | Note |
|---|---|---|---|
| **Fase 1 — Mobilità** | Dorsiflessione al muro | 12 rip/lato, 2 serie | Caviglia: 2s in alto, 2s in basso |
| **Fase 1 — Mobilità** | FMS Spalle | TBD (placeholder) | ❓ *Foto e descrizione tecnica da aggiungere* |
| **Fase 2 — Attivazione** | Ponte monopodalico | 6 rip/lato | Gluteo e stabilità bacino |
| **Fase 2 — Attivazione** | Shoulder Tap | 8 rip/lato | Core anti-rotazione |
| **Fase 4 — Avvicinamento** | Serie avvicinamento Squat | 2 serie × 6 rip | Bilanciere scarico, pausa 3-5s in buca |

### BLOCCO AM (Esercizi principali)

| # | Esercizio | Serie | Range Rip | RPE | Recupero | Note |
|---|---|---|---|---|---|---|
| **1** | **Squat** | 5 | 5-6 | 8-9 | 3-4 min | **LIFT PRIORITARIO** — Primo quando fresco. Ultimo set tecnico controllato (non cedimento puro) |
| **2** | **Stacco Rumeno** | 5 | 10-12 | 8-9 | 2-3 min | Complemento femorali. Eccentrica controllata 2-3s. Sensazione di stretching femorali e bacino neutro |

**Filler recupero (consigliati per recuperi ≥ 2.5 min):**
- Tra Squat: Band pull-apart leggero (15-20 rip, 30s)
- Tra Stacco Rumeno: Respirazione diaframmatica (4-5 cicli profondi)

### BLOCCO PM — Superset 1: Leg Curl + Ab Wheel

| # | Esercizio | Serie | Range Rip | RPE | Recupero Tra | Superset Con | Note |
|---|---|---|---|---|---|---|---|
| **3** | **Leg Curl al Cavo** | 4 | 10-12 | 8-9 | 0s | → Ab Wheel | Eccentrica 2-3s. Esecuzione immediata, no pausa |
| **—** | **Ab Wheel dalle ginocchia** | 5 | 8-10 | 8 | 60s | ← Leg Curl | Prime 2 avvicinamento + 3 lavoro. Ritorno controllato. Se affaticato, appoggiati |
| | | | | | **Recupero tra superset** | **60 secondi** | |

**Spiegazione superset:**
- Non è pausa tra i due esercizi
- Leg Curl (gamba) → subito Ab Wheel (core) → pausa 60s
- La riga dice: dopo Ab Wheel, riposa 60s prima di tornare a Leg Curl serie successiva

### STRETCHING FINALE (5-8 min)

| # | Esercizio | Durata | Cue |
|---|---|---|---|
| 1 | Standing Quad Pull | 30s/lato | Tallone verso gluteo, ginocchia vicine, bacino neutro |
| 2 | Half Kneeling Lunge | 30s/lato | Flessori anca, bacino neutro, anche leggermente avanti |
| 3 | Figure-Four Supino | 40s/lato | Glutei, piriforme. Sacro pesante a terra |

**Stato:** Segna `Fatto` o `Saltato` (non bloccante)

### BONUS MOBILITÀ ANCA (8-10 min, facoltativo dopo stretching)

| # | Posizione | Durata | Note |
|---|---|---|---|
| 1 | Half Kneeling Lunge | 2× 40s/lato | |
| 2 | Half Kneeling Lunge + Reach verticale | 2× 30s/lato | Braccio sopra la testa |
| 3 | Couch Stretch | 2× 40s/lato | Piede dietro il divano/panca |
| 4 | Figure-Four Supino | 2× 40s/lato | |
| 5 | Happy Baby | 2× 40s | |
| 6 | Leg Cradle Supino | 2× 30s/lato | |
| 7 | Supported Low Lunge | 2× 30s/lato | Con il TRX o al muro |

---

## 📍 GIORNO 2 — UPPER A (Martedì)

**Durata totale: ~90 minuti**
**Struttura:** Split AM/PM con superset nel blocco PM

### RISCALDAMENTO (12-15 min)

| Fase | Esercizio | Serie/Reps | Note |
|---|---|---|---|
| **Fase 1 — Mobilità** | T-spine rotation in quadrupedia | 8 rip/lato | Toracica: 1s pausa in rotazione |
| **Fase 1 — Mobilità** | FMS Spalle | TBD (placeholder) | ❓ *Foto e descrizione tecnica da aggiungere* |
| **Fase 1 — Mobilità** | Dorsiflessione al muro | 8 rip/lato | Appoggio stabile anche nei giorni upper |
| **Fase 2 — Attivazione** | Band Pull-Apart con elastico | 15 rip | Attivazione scapolare; pausa 1s ogni 5 rip |
| **Fase 2 — Attivazione** | Shoulder Tap | 8 rip/lato | Core anti-rotazione |
| **Fase 4 — Avvicinamento** | Mini-serie Trazioni | 2-3 serie | Progressive con elastico: elastico forte → medio → leggero (niente cedimento) |

### BLOCCO AM (Esercizi principali)

| # | Esercizio | Serie | Range Rip | RPE | Recupero | Note |
|---|---|---|---|---|---|---|
| **1** | **Trazioni** | 5 | 5-8 | 8-9 | 3-4 min | **LIFT PRIORITARIO** — Presa prona. Ultimo set cedimento tecnico controllato |
| **2** | **Panca** | 5 | 6-8 | 8-9 | 3-4 min | **LIFT PRIORITARIO** — Barra tocca petto, risale a braccia tese. Ultimo set tecnico controllato |

**Filler recupero:**
- Tra Trazioni: Retrazione scapolare passiva (5 cicli, 30s)
- Tra Panca: Dead hang passivo (20-30s, se barra disponibile)

### BLOCCO PM — Pulley separato + Superset 2: Band Pull-Apart + Curl Bicipiti

| # | Esercizio | Serie | Range Rip | RPE | Recupero Tra | Superset Con | Note |
|---|---|---|---|---|---|---|---|
| **3** | **Pulley Basso** | 5 | 10-12 | 8-9 | 0s | — | Tirata verticale bassa, controllo scapolare. Non la parte del superset qui sotto |
| | | | | | **Recupero tra Pulley e Band** | **90s** | Pulley è esercizio isolato prioritario |
| **4** | **Band Pull-Apart con rotazione esterna** | 3 | 15 | 8 | 0s | → Curl Bicipiti | Pausa 1s con mani ruotate fuori. Elastico medio |
| **—** | **Curl Bicipiti (manubri)** | 5 | 10-12 | 9 | 60s | ← Band Pull-Apart | Prime 2 avvicinamento + 3 lavoro. Eccentrica controllata 2-3s |
| | | | | | **Recupero tra superset** | **60 secondi** | |

**Spiegazione superset:**
- Pulley resta separato → recupero pieno prima del superset
- Band Pull-Apart + Curl Bicipiti = superset con pausa 60s

### STRETCHING FINALE (5-8 min)

| # | Esercizio | Durata | Cue |
|---|---|---|---|
| 1 | Doorway Pec Stretch | 30s/lato | Petto, gomito a 90°, spalla bassa |
| 2 | Overhead Lat Stretch | 30s/lato | Dorsali, piede lontano, inclinazione laterale marcata |
| 3 | Cross-Body Shoulder Stretch | 30s/lato | Deltoide posteriore e spalle, gomito in dentro |
| 4 | Wrist Flexor/Extensor Stretch | 20s/lato | Avambracci e polsi dopo presa e tirate |

**Stato:** Segna `Fatto` o `Saltato`

### BONUS MOBILITÀ ANCA (Como G1)

---

## 📍 GIORNO 3 — LEGGERO (Mercoledì)

**Tipo:** `light: true` — Niente coach, niente RIR, niente sessione guidata
**Durata totale: ~35 minuti**
**Struttura:** NO split AM/PM | NO superset | RPE 6-8

### RISCALDAMENTO (5 min, senza circuito)

| Esercizio | Reps/Durata | |
|---|---|---|
| Dorsiflessione al muro | 8 rip/lato | |
| 90/90 Hip Mobility | 8 passaggi/lato | |
| T-spine Rotation in quadrupedia | 8 rip/lato | |
| Ponte monopodalico | 6 rip/lato | |

### ESERCIZI (30 min, RPE 6-8)

| # | Esercizio | Serie | Range Rip | RPE | Recupero | Note |
|---|---|---|---|---|---|---|
| 1 | **Alzate Laterali** | 3 | 15-20 | 8 | 30-45s | Manubri leggeri. Alza fino a parallelo. 2s su 2s giù. Niente slancio |
| 2 | **Band Pull-Apart con rotazione esterna** | 2 | 20 | 6-7 | 30s | Volume extra leggero. Pausa 1s con mani ruotate fuori |
| 3 | **Fire Hydrant** | 2 | 20/lato | 6-7 | 30s | Bacino fermo, range piccolo e controllato. Gluteo medio |
| 4 | **Goblet Squat** | 2 | 12 | 6-7 | 45s | Manubrio al petto. Gomiti verso fuori, busto stabile |
| 5 | **TRX Row lento** | 2 | 12 | 6-7 | 45s | Fermo 2s con scapole strette. Eccentrica 3s in fuori |

**Alternativa G3**: il Fire Hydrant può essere alternato con **Clamshell** tramite toggle persistente nell'app. Stesse serie, RPE e recupero: 2x20/lato, RPE 6-7, 30s. Il Fire Hydrant del G4 resta fisso.

### PROTOCOLLO ANCA COMPLETO (8-10 min)

Questo protocollo è il cuore del G3 — il giorno migliore perché nessun compound pesante ha stancato il sistema.

| # | Posizione | Durata | |
|---|---|---|---|
| 1 | Half Kneeling Lunge | 2× 40s/lato | |
| 2 | Half Kneeling Lunge + Reach verticale | 2× 30s/lato | |
| 3 | Couch Stretch | 2× 40s/lato | |
| 4 | Figure-Four Supino | 2× 40s/lato | |
| 5 | Happy Baby | 2× 40s | |
| 6 | Leg Cradle Supino | 2× 30s/lato | |
| 7 | Supported Low Lunge | 2× 30s/lato | |

**✅ Nota importante:** Il G3 NON ha tracking nei Progressi, niente briefing coach, niente RIR guidato. È un giorno di recupero attivo e mobilità.

---

## 📍 GIORNO 4 — LOWER B (Giovedì)

**Durata totale: ~90 minuti**
**Struttura:** Split AM/PM con superset nel blocco PM

### RISCALDAMENTO (12-15 min)

| Fase | Esercizio | Serie/Reps | Note |
|---|---|---|---|
| **Fase 1 — Mobilità** | Dorsiflessione al muro | 8 rip/lato | Caviglia |
| **Fase 1 — Mobilità** | 90/90 Hip Mobility | 8 passaggi/lato | Anca |
| **Fase 1 — Mobilità** | Calf Raises con pallina | 12 rip | Piede e controllo adduttori |
| **Fase 2 — Attivazione** | Ponte monopodalico | 6 rip/lato | Gluteo e stabilità bacino |
| **Fase 2 — Attivazione** | Shoulder Tap | 6 rip/lato | Core anti-rotazione |
| **Fase 4 — Avvicinamento** | Serie avvicinamento Stacco | 3 serie | Scarico → 50% → 70-80%, 3-4 rip ciascuna |

### BLOCCO AM (Esercizi principali)

| # | Esercizio | Serie | Range Rip | RPE | Recupero | Note |
|---|---|---|---|---|---|---|
| **1** | **Stacco da Terra** | 5 | 4-5 | 8-9 | 3-4 min | **LIFT PRIORITARIO** — Ultime serie: solo RIR 0-1 se tecnica stabile. Non cedimento grezzo |
| **2** | **Affondi** | 5 | 10-12/gamba | 8-9 | 2-3 min | Manubri o bilanciere. Alternati: 10-12 dx → 10-12 sx → conteggio come 1 serie |

**Filler recupero:**
- Tra Stacco: Respirazione diaframmatica (5 cicli, 30-40s)
- Tra Affondi: Ankle circles (20 cerchi ogni piede, 30s)

### BLOCCO PM — Superset 3: Hyperextension + Fire Hydrant

| # | Esercizio | Serie | Range Rip | RPE | Recupero Tra | Superset Con | Note |
|---|---|---|---|---|---|---|---|
| **3** | **Hyperextension** | 5 | 12-15 | 8-9 | 0s | → Fire Hydrant | Prime 2 avvicinamento + 3 lavoro. Accessorio catena posteriore. Esecuzione immediata |
| **—** | **Fire Hydrant** | 3 | 15-20/lato | 8 | 60s | ← Hyperextension | Abduzione anca in quadrupedia, gluteo medio, bacino fermo |
| | | | | | **Recupero tra superset** | **60 secondi** | |

**Spiegazione superset:**
- Hyperextension → subito Fire Hydrant (pattern opposto: estensione anca → abduzione anca)
- Pausa 60s prima di tornare a Hyperextension serie successiva

### STRETCHING FINALE (5-8 min)

| # | Esercizio | Durata | Cue |
|---|---|---|---|
| 1 | Supine Assisted Leg Pull | 30s/lato | Femorali, bacino a terra, piede in dorsiflessione |
| 2 | Figure-Four Supino | 40s/lato | Glutei, piriforme |
| 3 | Cat-Cow Lento | 40s | Decompressione spinale post-stacco, 2s ogni posizione |
| 4 | Half Kneeling Lunge | 30s/lato | Flessori anca, bacino neutro |

**Stato:** Segna `Fatto` o `Saltato`

### BONUS MOBILITÀ ANCA (Come G1)

---

## 📍 GIORNO 5 — UPPER B (Sabato)

**Durata totale: ~90 minuti**
**Struttura:** Split AM/PM con superset nel blocco PM

### RISCALDAMENTO (12-15 min)

| Fase | Esercizio | Serie/Reps | Note |
|---|---|---|---|
| **Fase 1 — Mobilità** | T-spine Rotation in quadrupedia | 8 rip/lato | Toracica |
| **Fase 1 — Mobilità** | FMS Spalle | TBD (placeholder) | ❓ *Foto e descrizione tecnica da aggiungere* |
| **Fase 1 — Mobilità** | Dorsiflessione al muro | 8 rip/lato | Base stabile anche nel giorno upper |
| **Fase 2 — Attivazione** | Scapular Pull-Up | 5 rip | Depressione scapolare passiva (nessun piega di gomito) |
| **Fase 2 — Attivazione** | Band Pull-Apart con elastico | 15 rip | Attivazione scapolare, pausa 1s ogni 5 rip |
| **Fase 4 — Avvicinamento** | Serie avvicinamento Military Press | 2-3 serie | Bilanciere leggero e progressivo, niente cedimento |

### BLOCCO AM (Esercizi principali)

| # | Esercizio | Serie | Range Rip | RPE | Recupero | Note |
|---|---|---|---|---|---|---|
| **1** | **T-bar Row** | 5 | 8-10 | 8-9 | 3-4 min | **LIFT PRIORITARIO — Spostato da G2** — Tirata orizzontale prioritaria in Upper B |
| **2** | **Military Press** | 5 | 5-6 | 8-9 | 3-4 min | **LIFT PRIORITARIO** — Barra dalle clavicole, sopra la testa a braccia tese. Ultime serie RIR 0-1 se tecnica stabile |

**Filler recupero:**
- Tra T-bar Row: Retrazione scapolare (5 cicli, 30s)
- Tra Military Press: Dead hang passivo (20-30s)

### BLOCCO PM — Superset 4: Push-Up + Woodchop

| # | Esercizio | Serie | Range Rip | RPE | Recupero Tra | Superset Con | Note |
|---|---|---|---|---|---|---|---|
| **3** | **Push-Up** | 4 | 8-15 | 7-8 | 0s | → Woodchop | Corpo libero. Se stanca, continua serie successiva |
| **—** | **Woodchop (cavo o elastico)** | 3 | 10-12/lato | 8-9 | 45s | ← Push-Up | Spinta diagonale. Contrazione 1s in alto |
| | | | | | **Recupero tra superset** | **45 secondi** | |
| **4** | **Trazioni Supine** | 4 | 5-8 | 8 | 2-3 min | — | Presa supina. NON in superset. Recupero singolo |
| **5** | **Alzate Laterali** | 2 | 15 | 8 | 30-45s | — | Ultimo esercizio. Nessun superset |

**Spiegazione superset:**
- Push-Up → Woodchop, pausa 45s → serie successiva Push-Up
- Trazioni Supine: esercizio separato con recupero per sé
- Alzate Laterali: chiusura finale, fuori dal superset

### STRETCHING FINALE (5-8 min)

| # | Esercizio | Durata | Cue |
|---|---|---|---|
| 1 | Doorway Pec Stretch | 30s/lato | Dopo push-up |
| 2 | Overhead Triceps Stretch | 30s/lato | Spalle e tricipiti |
| 3 | Overhead Lat Stretch | 30s/lato | Dorsali, utile dopo trazioni supine |
| 4 | Band Dislocate Lento | 30s | Spalle e torace, elastico leggero, ritmo lento |

**Stato:** Segna `Fatto` o `Saltato`

### BONUS MOBILITÀ ANCA (Come G1)

---

## 📍 CARDIO PROGRAMMATO — Giovedì

**Durata totale: 30-60 minuti + mobilità**

### CARDIO — Quattro opzioni selezionabili

| Opzione | Durata | Zona FC | Note |
|---|---|---|---|
| **A — Corsa 30 min** | 30 min | Zona 2 (120-140) | Puoi parlare senza fiatone |
| **B — HIIT upper + core** | 30 min | Alta controllata | 30s lavoro / 15s pausa, 4-5 giri, niente sprint o salti |
| **C — Corsa lunga** | 50-60 min | Zona 2 (120-140) | Ritmo costante, cadenza 170+ passi/min |
| **D — Rucking** | 45-60 min | Zona 2 | Zaino 15-20 kg, progressione +5 min ogni 2 settimane |

### MOBILITÀ POST-CARDIO (10 min)

Dopo il cardio, oppure in un secondo momento della giornata:

| # | Esercizio | Durata |
|---|---|---|
| 1 | Dorsiflessione al muro | 2× 8 rip/lato |
| 2 | Alfredson eccentrico (per tibiale/polpaccio) | 2× 15s/lato |
| 3 | 90/90 Hip Mobility | 2× 8 passaggi/lato |
| 4 | T-spine Rotation in quadrupedia | 2× 8 rip/lato |
| 5 | Half Kneeling Lunge | 2× 30s/lato |
| 6 | Figure-Four Supino | 2× 40s/lato |

---

# RIEPILOGO TIMING E TEMPI DI RECUPERO

## Tempi di recupero base in app (secondo modalità guidata attuale)

### Per esercizi HEAVY (Squat, Stacco, Panca, Military, Trazioni, Stacco Rumeno, T-bar Row, Pulley, Glute Bridge Bilanciere)

| RIR | Tempo recupero |
|---|---|
| 0 | 2m 30s |
| 1 | 2m |
| 2 | 90s |
| 3+ | 75s |

### Per esercizi COMPOUND ACCESSORI (Affondi, Hyperextension, ecc)

| RIR | Tempo recupero |
|---|---|
| 0 | 90s |
| 1 | 75s |
| 2 | 60s |
| 3+ | 45s |

### Per esercizi MONOARTICOLARI (Curl, Alzate Laterali, Woodchop, Fire Hydrant, Ab Wheel, ecc)

| RIR | Tempo recupero |
|---|---|
| 0 | 60s |
| 1 | 60s |
| 2 | 45s |
| 3+ | 30s |

---

# SUPERSET ATTIVI — Riepilogo

| Giorno | Superset | Esercizio 1 | Esercizio 2 | Recupero tra superset | Nota |
|---|---|---|---|---|---|
| **G1** | Superset 1 | Leg Curl al Cavo (10-12) | Ab Wheel (5 serie: 2 avv. + 3 lavoro) | **60s** | Sequenza: Leg Curl → Ab Wheel → 60s → ripeti |
| **G2** | Superset 2 | Band Pull-Apart (20) | Curl Bicipiti (5 serie: 2 avv. + 3 lavoro) | **60s** | Sequenza: Band → Curl → 60s → ripeti. Pulley è esercizio separato |
| **G4** | Superset 3 | Hyperextension (5 serie: 2 avv. + 3 lavoro) | Fire Hydrant (15-20/lato) | **60s** | Sequenza: Hyper → Fire Hydrant → 60s → ripeti |
| **G5** | Superset 4 | Push-Up (8-15) | Woodchop (10-12/lato) | **45s** | Sequenza: Push-Up → Woodchop → 45s → ripeti. Trazioni Supine e Alzate Laterali restano separate |

---

# SPLIT AM/PM — Riepilogo

I giorni pesi sono organizzati in **AM (esercizi pesanti)** e **PM (blocchi in superset)**:

| Giorno | AM | PM |
|---|---|---|
| **G1** | Squat, Stacco Rumeno, Glute Bridge Bilanciere | Leg Curl + Ab Wheel (superset) |
| **G2** | Trazioni, Panca | Pulley, Band Pull-Apart + Curl Bicipiti (superset) |
| **G4** | Stacco da Terra, Affondi | Hyperextension + Fire Hydrant (superset) |
| **G5** | T-bar Row, Military Press | Trazioni Supine, Push-Up + Woodchop (superset), Alzate Laterali |

---

# ⚠️ DOMANDE DI CONFERMA PER L'AGENTE

Leggere il programma sopra e rispondere a questi punti:

## 1. Superset — Corretti i nomi? ✓ / ✗

- ❓ **G1:** "Leg Curl + Ab Wheel" rinominato **"Superset 1"** — Corretto?
- ❓ **G2:** "Band Pull-Apart + Curl Bicipiti" rinominato **"Superset 2"** — Corretto?
- ❓ **G4:** "Hyperextension + Fire Hydrant" rinominato **"Superset 3"** — Corretto?
- ❓ **G5:** "Push-Up + Woodchop" rinominato **"Superset 4"** — Corretto?

## 2. Split AM/PM — Esercizi divisi correttamente? ✓ / ✗

- ❓ **G1 AM:** Squat, Stacco Rumeno, Glute Bridge Bilanciere — Corretto? Manca qualcosa?
- ❓ **G1 PM:** Leg Curl + Ab Wheel in superset — Corretto?
- ❓ **G2 AM:** Trazioni, Panca — Corretto?  
- ❓ **G2 PM:** Pulley separato, poi Band Pull-Apart + Curl Bicipiti in superset — Corretto?
- ❓ **G4 AM:** Stacco da Terra, Affondi — Corretto?
- ❓ **G4 PM:** Hyperextension + Fire Hydrant in superset — Corretto?
- ❓ **G5 AM:** T-bar Row, Military Press — Corretto?
- ❓ **G5 PM:** Trazioni Supine separate, poi Push-Up + Woodchop in superset, poi Alzate Laterali — Corretto?

## 3. Ripetizioni — Sono quelle giuste? ✓ / ✗

- ❓ **G1:** Squat 5×5-6, Stacco Rumeno 5×10-12, Leg Curl 4×10-12, Ab Wheel 5×8-10 (prime 2 avvicinamento) — Corretto?
- ❓ **G2:** Trazioni 5×5-8, Panca 5×6-8, Pulley 5×10-12, Band 2×20, Curl 5×10-12 (prime 2 avvicinamento) — Corretto?
- ❓ **G4:** Stacco 5×4-5, Affondi 5×10-12/gamba, Hyper 5×12-15 (prime 2 avvicinamento), Fire Hydrant 3×15-20/lato — Corretto?
- ❓ **G5:** T-bar 5×8-10, Military 5×5-6, Push-Up 4×8-15, Trazioni Supine 4×5-8, Woodchop 3×10-12/lato — Corretto?

## 4. Tempi di recupero — Sono realistici? ✓ / ✗

- ❓ **Esercizi heavy:** 2m 30s (RIR 0) → 90s (RIR 2) — Realistico per il tuo tipo di allenamento?
- ❓ **Superset:** 45-60s tra i cicli — Tempo sufficiente per recuperare prima di tornare all'esercizio 1?
- ❓ **Filler recupero** (band pull-apart, respirazione, ecc): 30-40s — Consigliato? Utile nella pratica?

## 5. Giorno leggero G3 — RPE 6-7 è l'obiettivo giusto? ✓ / ✗

- ❓ RPE 6-7 = margine di 3-4 rip in riserva. Così la seduta rimane davvero leggera?
- ❓ Assenza totale di coach, RIR, e feedback sui carichi — OK? O serve feedback minimo?
- ❓ Protocollo anca completo 8-10 min dopo gli esercizi — Prioritario come dichiarato?

## 6. Placeholder — FMS Spalle da aggiungere? ✓ / ✗

- ❓ **FMS Spalle** appare in G1, G2, G5 (fase 1 mobilità) come `TBD (placeholder)` — Quando agregate foto e descrizione tecnica?
- ❓ Fino ad allora, l'app mostra il placeholder o salta la voce?

## 7. Esercizi rimossi — Confermi che sono davvero tolti? ✓ / ✗

Questi esercizi sono stati rimossi dalla V4 attuale:
- Hip Thrust Bilanciere (era G1 e G4)
- Squat Bulgaro (era G1, poi spostato, poi rimosso)
- Fitball Hamstring Curl (era G4)
- Face Pull (era G2)
- Tricipiti Cavo (era G5)
- Dip alle Parallele (era G5)

❓ Confermato che nessuno di questi va reinserito?

## 8. Altro?

- ❓ Qualcosa che manca o è incoerente nel programma?
- ❓ Varianti alternative per esercizi? (es. Push-Up su rialzo, Alternativa Trazioni con elastico, ecc) — Vanno documentate in un file separato?

---

## ✅ FIRMA CONFERMA

Una volta verificato tutto:

```
Agente PT Claude — Revisione CONFERMATA il _____ / _____ / 2026

Modifiche eventualmente suggerite dal PT:
- [...]

Data di attivazione programma aggiornato: _____
```
